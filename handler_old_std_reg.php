...
if (isset($_POST['enroll'])) {
    $stdNo = mysqli_real_escape_string($con, trim($_POST['stdNumber']));

    $sql = mysqli_query($con, "SELECT StdNumber FROM student WHERE StdNumber='$stdNo'") or die("<b>Error:</b> Something went wrong.<br/>" . mysqli_error($con));;
    $count = mysqli_num_rows($sql);

    if ($count > 0) {
        $fetch = mysqli_fetch_assoc($sql);
        if ($fetch["StdNumber"] ==  $stdNo) {
        ...
        if (is_uploaded_file($_FILES['userImage']['tmp_name'])) {
            ...
            // COOMENT OUT FOR ALL STUDENTS REGUSTRATION
            $block = $_POST['block'];
            ...
            $courseIdQuery = mysqli_query($con, "SELECT CourseId, CourseName FROM course WHERE CourseName = '$course'") or die("<b>Error:</b> Something went wrong in course ID.<br/>" . mysqli_error($con));;
            $count = mysqli_num_rows($courseIdQuery);

            if ($count > 0) {
                $fetch = mysqli_fetch_assoc($courseIdQuery);
                if ($fetch["CourseName"] ==  $course) {
                    $courseId = $fetch["CourseId"];

                    //$vacsCardImage = vacsCard();
                    $sql = "insert into student(`StdIdPicture`, `StdNumber`, `StdEmailAdd`, `Scholarship`, `StdLastName`, `StdFirstName`, `StdMiddleName`, `ContactNumber`, `SY`, `Sem`, `YearLevel`, `Block`, `Course`, `DateOfReg`, `CivilStatus`, `Age`, `Sex`, `DateOfBirth`, `PlaceOfBirth`, `Nationality`, `Religion`, `HomeHouseNo`, `HomeStreet`, `HomeBarangay`, `HomeTown`, `HomeCity`, `HomeZipCode`, `ProvHouseNo`, `ProvStreet`, `ProvBarangay`, `ProvTown`, `ProvCity`, `ProvZipCode`, `Fathers_Surname`, `Fathers_Given_Name`, `Fathers_Middle_Name`, `FathersOccupation`, `FathersContactNo`, `Mothers_Maiden_Name`, `Mothers_Given_Name`, `Mothers_Middle_Name`, `MothersOccupation`, `MothersContactNo`, `GuardiansName`, `Relationship_To_Guardian`, `GuardiansOccupation`, `GuardiansContactNo`, `GuardiansHouseNo`, `GuardiansStreet`, `GuardiansBarangay`, `GuardiansTown`, `GuardiansCity`, `GuardiansZipCode`, `FinancialSupport`, `StdNature`, `ElemName`, `ElemAddress`, `ElemHonors`, `ElemSY`, `JhName`, `JhAddress`, `JhHonors`, `JhSY`, `ShName`, `ShAddress`, `ShHonors`, `ShSY`, `CollegeName`, `CollegeAddress`, `CollegeSY`, `Vaccine`) 
                values('" . $imgData . "',
                '" . $stdNumber . "',
                '" . $email . "',
                '" . $scholarship . "',
                ...
                '" . $vaccine . "'  
                    ...
                    if (isset($current_id)) {
                        $_SESSION['status'] = "Succesfully Registered";
                        header("Location: registered.php");

                        /* 
                //// Uncomment this for all students resgitrration ///
            ...
            $_SESSION['noID'] = "You must upload your 2x2 ID picture";
            header("Location: registered.php");
        }
    }
}
